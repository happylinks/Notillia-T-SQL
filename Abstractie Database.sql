...
--Test(s)
CREATE VIEW ComponistsWithJob AS
SELECT c.componistId, c.naam AS ComponistNaam, c.geboortedatum, m.schoolId, m.naam AS MuziekschoolNaam, m.plaatsnaam 
FROM Componist c
	INNER JOIN Muziekschool m ON c.schoolId = m.schoolId;
GO

SELECT * FROM dbo.ComponistsWithJob;
GO