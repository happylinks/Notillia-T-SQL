...
CREATE table notillia.datatypeConversion(
OriginalDatatype nvarchar(256) not null,
ConvertedDatatype nvarchar(max) not null,

CONSTRAINT PK_datatypeconversion PRIMARY KEY (OriginalDataType)
)
go

insert into notillia.datatypeConversion
values
('varchar',  'VARCHAR(<character_maximum_length>)'),
('nvarachar', 'VARCHAR(<character_maximum_length>) CHARSET utf8'),
('numeric', 'NUMERIC(<numeric_precision>, <numeric_scale>)'),
('char', 'CHAR(<character_maximum_length>)'),
('datetime', 'DATETIME'),
('int', 'INT'),
('varbinary', 'VARBINARY(<character_maximum_length>)'),
('tinyint', 'TINYINT'),
('smallint', 'SMALLINT'),
('bigint', 'BIGINT'),
('float', 'FLOAT(<numeric_precision>)'),
('decimal', 'DECIMAL(<numeric_precision>, <numeric_scale>)'),
('datetime2', 'DATETIME'),
('date', 'DATE'),
('time', 'TIME'),
('binary', 'BINARY'),
('nchar', 'CHAR(<character_maximum_length>) + CHARSET utf8'),
('money', 'DECIMAL(<numeric_precision>, 2)'),
('smallmoney', 'DECIMAL(<numeric_precision>, 2)'),
('real', 'REAL')
...
declare @convertedDatatype VARCHAR(MAX) = '';

select @ConvertedDatatype += ConvertedDatatype
from notillia.datatypeConversion
where OriginalDatatype in (SELECT Data_type + '' from notillia.columns where table_name = @tableName and column_name = @columnName)

print @convertedDatatype

set @convertedDatatype = REPLACE(@convertedDataType, '<character_maximum_length>', 
(select character_maximum_length + '' from notillia.columns  where table_name = 'componist' and column_name = 'naam' FOR xml path('')))

set @convertedDatatype = REPLACE(@convertedDataType, '<numeric_precision>', 
(select numeric_precision + '' from notillia.columns  where table_name = 'componist' and column_name = 'naam' FOR xml path('')))

set @convertedDatatype = REPLACE(@convertedDataType, '<numeric_scale>', 
(select numeric_scale + '' from notillia.columns  where table_name = 'componist' and column_name = 'naam' FOR xml path('')))
